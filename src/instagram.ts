...
export class InstagramFetcher {
     ...
     */
    private async executeInPage(url: string, method: string = 'GET'): Promise<any> {
        const script = `
        (async function() {
            try {
        ...
        try {
            // neo eval sends "Evaluating in: <url>" to stderr, stdout is the raw result
            // First try parsing the entire output directly
            let parsed: any;
            try {
                parsed = JSON.parse(output);
            } catch {
                // Fallback: if there's a prefix line, take the last line
     ...
     */
    async getUserPosts(userId: string, maxPosts: number = 50): Promise<any[]> {
        ...
        try {
            while (allItems.length < maxPosts) {
                let url = `https://www.instagram.com/api/v1/feed/user/${userId}/?count=12`;
            ...
            console.log(`[IG] Found ${allItems.length} posts for user ${userId}`);
